...
def judge_hands(hands_list):
    ...
    for hand in hands_list:
        ...
        # loop over card types and count how many are observed
        counts = [card_ranks.count(card) for card in card_types]
        # boolean tests to determine type of hand, append (card_ranks, bid) to 
        # respective hand types
        # fullhouse and two pair are the most difficult to determine so check 
        # those first
        if counts in [[3,2],[2,3]]:
            hand_types['fullhouse'].append((card_ranks,int(hand[1]),cards[card_ranks[0]]+cards[card_ranks[1]]+cards[card_ranks[2]]+cards[card_ranks[3]]+cards[card_ranks[4]]))
        elif counts in [[2,2,1],[2,1,2],[1,2,2]]:
            hand_types['twopair'].append((card_ranks,int(hand[1]),cards[card_ranks[0]]+cards[card_ranks[1]]+cards[card_ranks[2]]+cards[card_ranks[3]]+cards[card_ranks[4]]))
        ...
        # appropriate dictionary key's list
            ...
            hand_types[max_count].append((card_ranks,int(hand[1]),cards[card_ranks[0]]+cards[card_ranks[1]]+cards[card_ranks[2]]+cards[card_ranks[3]]+cards[card_ranks[4]]))
